...
mask = mask.sel(time=verification_year)

fill = fill.expand_dims(veriset=[veriset])
mask = mask.expand_dims(veriset=[veriset])
fill.to_dataset('variable').to_netcdf(f'{esapath}{testcase}/verification/set{veriset}/data_{label}_del.nc')
mask.to_dataset('variable').to_netcdf(f'{esapath}{testcase}/verification/set{veriset}/mask_crossval_del.nc')
